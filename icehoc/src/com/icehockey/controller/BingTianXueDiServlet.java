	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (session.getAttribute("user") == null) {
			...
			if (request.getParameter("operateType") != null) {
				...
				if ("zhukongToBingTianXueDi".equals(operateType)) {// 如果操作类型是主控页面到浇冰必拜主页面，则取出场地表中的所有场地信息
					...
					String nameString = request.getParameter("searchString");
					teams = teamService.queryTeamsByNameString(nameString);
					session.setAttribute("teams", teams);
					map.put("result", "0");
					map.put("ok", "3");
				} else if ("competitionSearch".equals(operateType)) {// 如果操作类型是主控页面到浇冰必拜主页面，则取出场地表中的所有场地信息
					String nameString = request.getParameter("searchString");
					matchs = matchService.getMatchsByStr(nameString);
					...
					match = matchService.getMatchByCompetitionId(id);
					...
					boolean f = teamService.addTeam(teamName, buildTime, construction);
					if (f) {
						teams = teamService.queryTeams();
						session.setAttribute("teams", teams);
						map.put("result", "0");
					...
					boolean f = competitionService.addCompetition(teamAId, teamBId, rinkId, time, remark,true);
					if(f){
						matchs = matchService.getAllOfficialCompetitions();
						session.setAttribute("matchs", matchs);
						map.put("result", "0");
				...
				}else if ("competitionAddtBtn".equals(operateType)) {// 如果操作类型是主控页面到浇冰必拜主页面，则取出场地表中的所有场地信息
		...
		if ("0".equals(map.get("result"))) {// 登录成功，且不是第一次登陆
			...
			if ("1".equals(map.get("ok"))) {
				...
				writer.println(
						"<script language='javascript'>alert('添加成功！');window.location.href='./views/part3/bisaiyemian.jsp'</script>");
			}else if ("13".equals(map.get("ok"))) {
				writer.println(
						"<script language='javascript'>window.location.href='./views/part3/addCompetition.jsp'</script>");
			}
